...
function Stick(params) {
    if (params === 'sticker' ) {
        'ok'
    }else{error.massage('Выберите эмоджи')}
}
...
// bot.on('sticker', (ctx) => ctx.reply('👍'));
// bot.hears('hi', (ctx) => ctx.reply('Hey there'));
// function questionWithAnswerOptions(btn, question, options){
...
//     }
// })
// bot.on('photo', (ctx) => ctx.reply('Оберіть емодзі, яке передає ваше ставлення до шампуню Garnier.'))
